...

{
  ...
  git-hooks.hooks = {
    ...
    # Only runs tests that don't require Docker services (S3/MinIO)
    unit-tests = {
      enable = true;

      name = "Unit tests";

      # Run only httpfs and secrets tests that don't require Docker services
      # Skips tests requiring: S3_TEST_SERVER_AVAILABLE, HUGGING_FACE_TOKEN, etc.
      entry = "bash -c 'if [ -f build/release/test/unittest ]; then build/release/test/unittest \"[httpfs]\" \"[secrets]\" || true; else echo \"Tests not built yet, skipping...\"; fi'";
