...
unique_ptr<HTTPParams> HTTPFSUtil::InitializeParameters(optional_ptr<FileOpener> opener,
	...
	FileOpener::TryGetCurrentSetting(opener, "webdav_debug_logging", result->webdav_debug_logging, info);
	FileOpener::TryGetCurrentSetting(opener, "webdav_max_retries", result->webdav_max_retries, info);
	FileOpener::TryGetCurrentSetting(opener, "webdav_streaming_threshold_mb", result->webdav_streaming_threshold_mb,
	                                 info);
...
static optional_ptr<HTTPMetadataCache> TryGetMetadataCache(optional_ptr<FileOpener> opener, HTTPFileSystem &httpfs) {
	...
	Value http_metadata_cache_val;
	bool use_shared_cache = false;
	if (FileOpener::TryGetCurrentSetting(opener, "enable_http_metadata_cache", http_metadata_cache_val)) {
		use_shared_cache = http_metadata_cache_val.GetValue<bool>();
	}
	if (use_shared_cache) {
